    ...
    return (
        <div className="app-shell" style={jamThemeVars}>
            ...
            <header className="top-bar">
                <div>
                    <p className="eyebrow">jamming.fun / Artist Studio</p>
                    <h1>Beat Creation Studio</h1>
                    <p className="subtitle">Create beats, manage rounds, and share your room with listeners.</p>
                </div>
                <div className="top-actions">
                    {room && (
                        <div className="share-badge" onClick={copyRoomCode} title="Click to copy share link">
                            <span className="share-badge-label">Room Code</span>
                            <span className="share-badge-code">{room.code}</span>
            ...
            <main className="layout">
                <section className="main-column">
                    <Panel title="Drum Machine" subtitle="Create your beat pattern">
                    ...
                    <Panel title="Round Controls" subtitle="Commit → Predict → Lock → Reveal → Settle">
                ...
                <aside className="side-column">
                    <Panel title="Test Prediction" subtitle="Simulate a user prediction from artist view">
                    ...
                    <Panel title="Room / Round Status" subtitle="Session status">
                    ...
                    <Panel title="Leaderboard / Rewards" subtitle="Track token units (V1 rewards)">
                        {settlement ? (
                            <>
                            ...
                            <p className="placeholder-line">No settlement yet.</p>
                        )}
                    </Panel>

                    <Panel title="Integrations" subtitle="MagicBlock + Audius + Blinks">
                        <div className="integration-list">
                            ...
                            <div>
                                ...
                                <Pill tone={health?.integrations?.magicBlock?.mode === 'real' ? 'success' : health?.featureFlags?.enableMagicBlock ? 'accent' : 'danger'}>
                                    {health?.integrations?.magicBlock?.mode ?? (health?.featureFlags?.enableMagicBlock ? 'enabled' : 'disabled')}
                                </Pill>
                            </div>
                            <div>
                                <span>Audius</span>
                                <Pill tone={health?.integrations?.audius?.mode === 'real' ? 'success' : health?.featureFlags?.enableAudius ? 'accent' : 'danger'}>
                                    {health?.integrations?.audius?.mode ?? (health?.featureFlags?.enableAudius ? 'enabled' : 'disabled')}
                                </Pill>
                            </div>
                            <div>
                                <span>Blinks</span>
                                <Pill tone={health?.integrations?.blinks?.mode === 'real' ? 'success' : health?.featureFlags?.enableBlinks ? 'accent' : 'danger'}>
                                    {health?.integrations?.blinks?.mode ?? (health?.featureFlags?.enableBlinks ? 'enabled' : 'disabled')}
                                </Pill>
                            </div>
                            {blinkUrls ? (
                    ...
                    <Panel title="Activity Feed" subtitle="Event trace">
                        {activity.length === 0 ? (
